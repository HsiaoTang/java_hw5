...
public class calendar_display_sd_admin extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					calendar_display_sd_admin frame = new calendar_display_sd_admin();
	...
	public static void setS_d(String s_d) {
		calendar_display_sd_admin.s_d = s_d;
	}
	
	public static String getS_d() {
		return calendar_display_sd_admin.s_d;
	}
	
	/**
	 * Create the frame.
	 */
	public calendar_display_sd_admin() {
		...
		l_y.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				d_p.removeAll();
				util_member.regen_cal(d_p, y_tf, m_tf, calendar_display_sd_admin.this, order_rec.s_d_tf);
				d_p.revalidate();
				d_p.repaint();
			}
		});
...
;					date.set(Calendar.YEAR, y);
				...
				public void mousePressed(MouseEvent e) {
					...
					calendar_display_sd_admin.s_d = y_tf.getText() + "-" + String.format("%02d", Integer.parseInt(m_tf.getText().replace("月", "")))
											+ "-" + String.format("%02d", Integer.parseInt(m_d.getText()));
					order_rec.s_d_tf.setText(s_d);
					dispose();
				}
